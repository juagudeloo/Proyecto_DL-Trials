...
def main():
    ...
    for i in range(2):
        for j in range(2):
            ax[i,j].plot(np.arange(0,max_height,1)+1, atm_params[max_x_plot, max_z_plot, i*2+j], label = "Pparámetros generados")
            ...
            ax[i,j].set_xlabel("Píxeles de altura", fontsize = fontsize)
