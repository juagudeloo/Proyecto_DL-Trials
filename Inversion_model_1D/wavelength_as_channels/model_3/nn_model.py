...
class InvModel1(nn.Module):
    def __init__(self, in_shape, out_shape, hidden_units):
        ...
        nn.Conv1d(in_channels=in_shape, out_channels=72, kernel_size = 6, stride=1, padding=padding),
        nn.ReLU(),
        nn.Flatten(),
        nn.Dropout(p=0.5, inplace=False),
        nn.Linear(in_features = 72, out_features = out_shape))
    def forward(self, x):
...
def validation_visual(generated_quant_list:list, ref_quant:np.ndarray, epochs_to_plot:list, images_out:str, titles:list):
    ...
    for ni in range(frames):
        ...
        fig.savefig(images_out+f"visualization_{epochs_to_plot}.pdf")
